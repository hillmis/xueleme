...
(function (window, document) {
    ...
    const CONFIG = {
        ...
        UPDATE: {
            ...
            // 兼容旧配置（保留第一个为 HOST）
            HOST: 'https://raw.githubusercontent.com/hillmis/versionControl/main',
    ...
    class SystemApp {
        constructor() {
            ...
            this.state = {
                ...
                appName: typeof webapp !== 'undefined' ? webapp.getname() : '学了么',
        ...
        async _fetchRemoteUpdateData() {
            ...
            let lastErr = null;
            for (const updateUrl of tryUrls) {
                console.log(`[Update] 尝试获取元数据: ${updateUrl}`);
                try {
                    const response = await fetch(updateUrl);
                    if (!response.ok) {
                        lastErr = new Error(`HTTP错误: ${response.status} @ ${updateUrl}`);
                        continue;
                    }
                    const data = await response.json();
                    if (!data.versionCode || !data.versionName) {
                        lastErr = new Error('无效的版本数据格式');
                        continue;
                    }
                    return data;
                } catch (err) {
        ...
        <div class="content-container">
            <div class="Sponsor-logo-section" style="display:flex;flex-direction:column;align-items:center;">
                <div class="logo-box-animated">
                    <img src="https://s3.bmp.ovh/imgs/2026/01/13/6cd940745fbd68ac.jpg" alt="Logo" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
